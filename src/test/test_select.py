...
@pytest.mark.parametrize(("inputargs, outputargs"),[
	(['#20 == 1',open('./test/test.csv'), sys.stdout, False, '|', None],'The column offset "#20" should be in the range (0, 15)'),
	(['#test == 1',open('./test/test.csv'), sys.stdout, False, '|', None],'column offset "#test" cannot be a string as input data has no header'),
	(['#test == 1',open('./test/test.csv'), sys.stdout, True, '|', None], 'column offset "#test" entered is not found in input data header'),
	(['#2 == 1',open('./test/test.csv'), sys.stdout, True, ',', None], 'Did you miss passing delimiter arg?')
...
def test_col_validation_positive():
	inputargs = ['#1 == 3',open('./test/test.csv'), sys.stdout, False, '|', None]
